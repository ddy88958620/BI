...
$ETL::INVOKER = "${ETL::ETL_BIN}/etlslave_unix.pl";
...
#      $sqlText1 = "SELECT JobStatus FROM ${ETL::ETLDB}ETL_Job_Status" .
...
sub getControlFileInfo
{
   ...
   if ( $ETL::NameMode == $ETL::SHORT_NAME_MODE ) {
      ...
      $source = substr($file, 4, (length($file) - 12));
      $txdate = substr($file, (length($file) - 8), 8);
   }
   elsif ( $ETL::NameMode == $ETL::EVA_NAME_MODE ) {
      $source = substr($file, 4, (length($file) - 13));
      $txdate = substr($file, (length($file) - 8), 8);
   }
...
sub getJobStream
{
   ...
   my $sqlText = "SELECT Stream_System, Stream_Job" .
...
sub connectETL
{
   ...
   $dbh = DBI->connect("dbi:Teradata:${ETL::ETL_DSN}", $user, $decodepass) ;
