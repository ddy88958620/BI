...
# Argument : none
# Modified by Bill Wang 2008-1-3
#       Add JOB priority control function
###############################################################################
...
sub sortControlFile
{
	 my $L1 = substr($a, 0, 2);
	 my $L2 = substr($b, 0, 2);
   my $job1 =  substr($a, 2, (length($a) - 13 - 2));
   my $job2 =  substr($b, 2, (length($b) - 13- 2));
...
# Add by Bill Wang for JOB Priority Control, 2008-1-2 11:06
# Get priority of each job

#	my $sql_cmd = "SELECT Job_Priority FROM ${ETL::ETLDB}ETL_Job " .
#						    "WHERE	ETL_System = ? AND ETL_Job = ?";

# Change get job_priority by ETL_System and Conv_File_Head, maybe Conv_File_Head<>ETL_Job   2008-12-26 16:42
	my $sql_cmd = "SELECT Job_Priority FROM ${ETL::ETLDB}ETL_Job a, ${ETL::ETLDB}etl_job_source b " .
						    "WHERE	a.ETL_System = b.ETL_System AND a.ETL_Job = b.ETL_Job and a.ETL_System = ? AND b.Conv_File_Head = ?";

  my $sth = $dbCon->prepare($sql_cmd);

	for (my $i=0; $i < $n; $i++) {
	  my $sys = substr($tempList[$i], 0, 3);
	  my $job = substr($tempList[$i], 4, length($tempList[$i])-17);
  	$sth->execute($sys, $job);
	  my ($Level) = $sth->fetchrow();
		$tempList[$i] = sprintf("%02d", $Level) . $tempList[$i];
	}
########### Add by Bill Wang for JOB Priority Control
  ...
  my $currentJob = getCurrentJobCount();
   ...
   for (my $i=0; $i < $n; $i++)
   {
      ...
      $filename = $dirFileList[$i];
			$filename = substr($filename, 2);   # The 2 leader chars is Priority Level
...
#              "   ORDER BY SeqID";
   ...
   $sqlText = "SELECT SeqID, ETL_System, ETL_Job, CAST(TXDate AS CHAR(10))" .
              "   FROM ${ETL::ETLDB}ETL_Job_Queue" .
