...
# perl can support file-size > 2GB maybe the following line need to be removed
...
# Removed by Bill Wang 2009-3-25 15:31
#      if ( $size < 0 ) {
...
sub getPriorCalendarDate
{
   ...
   if ( $checkmode eq "0") {
      $sqlText = 
			" SELECT MAX(CAST(CAST(CalendarYear as format '9999') as char(4)) ||    \n".
			"		   CAST(CAST(CalendarMonth as format '99') as char(2)) ||       \n".
			"	       CAST(CAST(CalendarDay as format '99') as char(2)) )          \n".
			" FROM ${ETL::ETLDB}DataCalendar                                        \n".
			" WHERE ETL_System = '$sys' AND ETL_Job = '$job'                        \n".
			"      AND ( CAST(CAST(CalendarYear as format '9999') as char(4)) ||    \n".
			"            CAST(CAST(CalendarMonth as format '99') as char(2)) ||     \n".
			"            CAST(CAST(CalendarDay as format '99') as char(2)) ) <='$txdate' ";
	} else {
      $sqlText = 
			" SELECT MAX(CAST(CAST(CalendarYear as format '9999') as char(4)) ||         \n".
			"		   CAST(CAST(CalendarMonth as format '99') as char(2)) ||            \n".
...
sub compareCurrentTxDate
{
   ...
   $sqltext = "SELECT Cast(Last_TxDate AS CHAR(10))" . 
              "   FROM ${ETL::ETLDB}ETL_Job" . 
              "      WHERE ETL_System = '$sys'" .
              "        AND ETL_Job = '$job'";

#   $sqltext = "SELECT Last_TxDate" . 
#              "   FROM ${ETL::ETLDB}ETL_Job" . 
#              "      WHERE ETL_System = '$sys'" .
#              "        AND ETL_Job = '$job'";
...
sub processControlFile
{
   ...
   if ( compareCurrentTxDate($dbCon, $sysName, $jobName, $txDate) == $FALSE ) {
      ...
      $eventDesc = "[$sysName], [$jobName] has received a control file, whose txdate is less then the current txdate";
      ETL::insertEventLog($dbCon, "RCV", "M", "$eventDesc");

      removeJobEventRecord($sysName, $jobName);

      ETL::showTime(); print "Generate data file has less than current TxDate message...\n";
      writeMessageNotification($sysName, $jobName, $txDate,
                               "Receiving",
                               "Job [$sysName,$jobName] has received a control file, whose txdate is less then the current txdate",
                               "");
