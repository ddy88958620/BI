...
#检查参数1是否在列表文件中存在。正则表达式中:\<,\>和\b,\b都是单词边界的意思
...
#读取卸数日期控制文件,获取卸数日期。-s : 文件存在，且size大于0
...
# $$表示当前SHELL进程-pid; $!表示shell最后运行的后台进程-pid; $?表示最后运行的命令的结束代码-返回值
# $* 表示命令行参数列表"$1 $2 ..."; $@ 表示"$1" "$2" "$3" ...
echo $$ > $TMPDIR/$PNAME
# touch $TMPDIR/$PNAME
...
TMPDT=`date_add $ULDT -1`     #计算卸数日期前一日,执行不成功是提供的参数1,即日期值不合法
if [ $? -ne 0 ]
then
...
#卸数日期翻牌
ULDT=`date_add $ULDT +1`     #计算下一卸数日期,$?=0,表示命令执行成功
if [ $? -ne 0 ]
then
